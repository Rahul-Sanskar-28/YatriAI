...
import { Sparkles, Lightbulb, MapPin, Calendar, Camera, Coffee, Sun, Moon, Umbrella, Mountain } from 'lucide-react';
import { Marquee } from '../magicui/Marquee';
import { aiTips } from '../../data/mockData';

const tipIcons = [Lightbulb, MapPin, Calendar, Camera, Coffee, Sun, Moon, Umbrella, Mountain, Sparkles];
...
const TipCard = ({ tip, index }: { tip: string; index: number }) => {
  ...
  return (
    <div className="flex items-center gap-3 px-5 py-3 mx-2 bg-white/10 backdrop-blur-sm rounded-full border border-white/20 hover:bg-white/20 transition-all duration-300 group cursor-default">
...
const AITipsMarquee: React.FC = () => {
  ...
  const allTips = [...aiTips, ...aiTips];

  return (
    <div className="relative bg-gradient-to-r from-green-600 via-emerald-500 to-teal-500 py-6 overflow-hidden">
      ...
      <div className="absolute left-0 top-0 bottom-0 w-24 bg-gradient-to-r from-green-600 to-transparent z-10 pointer-events-none" />
      ...
      <div className="flex items-center justify-center gap-3 mb-4 relative z-20">
        <div className="flex items-center gap-2 px-4 py-1.5 bg-white/20 backdrop-blur-sm rounded-full border border-white/30">
          ...
          <span className="text-white font-semibold text-sm">AI Travel Tips</span>
          <span className="px-2 py-0.5 bg-yellow-400/20 text-yellow-200 text-xs rounded-full">Live</span>
        </div>
      </div>
