  ...
  const fetchApprovedTours = async () => {
    try {
      ...
      if (response.success && response.data) {
        setApprovedTours(response.data);
      }
    } catch (error) {
  ...
  const handleSpeak = async (text: string) => {
    ...
    try {
      if (elevenLabsConfigured) {
        ...
        await voiceService.speakWithBrowserTTS(text, 'en');
      }
    } catch (error) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="hidden md:flex items-center gap-4 text-sm text-gray-600 dark:text-gray-400">
              ...
              {approvedTours.length > 0 && (
                <div className="flex items-center gap-1">
                  <Map className="w-4 h-4 text-kolkata-terracotta" />
                  <span>{approvedTours.length} Tours</span>
                </div>
              )}
            </div>
            {approvedTours.length > 0 && (
              ...
              >
                ...
                {showTours ? 'Show Artworks' : 'Show Tours'}
              </motion.button>
            )}
          </div>
        </div>
