...
const fetchTrainAvailability = async (booking: TrainBooking): Promise<TrainOption[] | null> => {
  const apiBase = import.meta.env.VITE_INDIAN_RAIL_API_BASE || 'http://localhost:3000/trains';
  ...
  try {
      const url = `${apiBase}/betweenStations?from=${fromCode}&to=${toStation}`;
      const response = await fetch(url);
    if (!response.ok) {
        console.warn(`Train API returned ${response.status} for ${fromCode} -> ${toStation}`);
        continue;
    }
    ...
    const data = await response.json();
      ...
      trains.slice(0, 6).forEach((t: any) => {
        ...
        results.push({
          from: t.from_stn_name || fromCode,
          to: t.to_stn_name || toStation,
